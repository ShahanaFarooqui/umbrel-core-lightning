...
class LightningController {
  callMethod(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      lnMessage.call(req.body.method, req.body.params)
      .then((commandRes: any) => {
        logger.info('Controller received response for ' + req.body.method + ': ' + JSON.stringify(commandRes));
        res.status(200).json(commandRes);
      })
      .catch((err: any) => {
        logger.error('Controller caught lightning error from ' + req.body.method + ': ' + JSON.stringify(err));
